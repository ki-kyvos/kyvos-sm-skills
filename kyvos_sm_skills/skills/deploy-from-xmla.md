...
# Skip relationships where left/right columns don't exist in the actual datasets.
# This prevents DRD creation failures from invalid column references.
validated_rels = []
...
# Map XMLA names to CamelCase server names for fact_dataset_names
...
drd_artifact = compile_drd_artifact(
    ...
    relationships=validated_rels,     # only validated relationships
...
# The Kyvos server may need a few seconds to process the DRD before validation returns VALID.
...
# The Kyvos server may need a few seconds to process the SM before validation returns VALID.
for _attempt in range(1, _max_validation_retries + 1):
